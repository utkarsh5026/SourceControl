...
func TestFileObjectStore_WriteAndReadTree(t *testing.T) {
	...
	// Create tree entries
	entry1, err := tree.NewTreeEntry(string(tree.EntryTypeRegularFile), "file1.txt", hash1.String())
	if err != nil {
		t.Fatalf("failed to create tree entry 1: %v", err)
	}

	entry2, err := tree.NewTreeEntry(string(tree.EntryTypeRegularFile), "file2.txt", hash2.String())
	if err != nil {
